...
try {
    ...
    // Insertar tipo de evento (sin imagen)
    $stmt = $conn->prepare("
        INSERT INTO TIPOS_EVENTO (NOM_TIPO_EVE)
        VALUES (?)
    ");
    ...
    if ($nuevo_requisito) {
        $stmt_req = $conn->prepare("
            INSERT INTO REQUISITOS (NOM_REQ, DES_REQ, TIPO)
            VALUES (?, ?, 'TEXTO_CORTO')
        ");
    ...
    if (!empty($requisitos)) {
        $stmt_rel = $conn->prepare("
            INSERT INTO TIPOS_EVENTO_REQUISITOS (ID_TIPO_EVE, ID_REQ)
            VALUES (?, ?)
        ");
